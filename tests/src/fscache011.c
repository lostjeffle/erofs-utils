...
static int local_process_read_req(int devfd, struct cachefiles_msg *msg)
{
	...
	/* wait for the state switch of devfd */
	sleep(1);

	/* check writing to cache file when cache is already dead */
	ret = pwrite(dst_fd, readbuf, len, read->off);
	if (ret != len) {
		printf("expected: write dst image failed\n");
		/* continue */
	} else {
		printf("unexpected: writing to cache file when cache is already dead\n");
	}
	...
	if (ret < 0) {
		printf("expected: send cread failed\n");
		/* continue */
	} else {
